...
class DBSCANHotspotDetector:
    ...
    def detect_hidden_hotspots(self, sensor_df, citizen_df, satellite_df):
        ...
        if sensor_df is not None and not sensor_df.empty:
            for _, r in sensor_df.iterrows():
                ...
                if pm25 > 60.0: # Only include high pollution points for DBSCAN hotspot finding
        ...
        for cid in unique_clusters:
            ...
            # Check if this cluster is an UNMONITORED hidden hotspot (no ground sensors nearby!)
            has_sensor = 'Ground Sensor' in sources_present
            is_hidden = not has_sensor

            # Approximate ward association based on lat/lon
            if center_lat > 13.0:
                ward_name = "Peenya & Dasarahalli Industrial Corridor"
            ...
            cluster_summaries.append({
                ...
                "status_label": "🚨 HIDDEN UNMONITORED HOTSPOT" if is_hidden else "⚠️ MONITORED HOTSPOT",
    ...
    def overlay_hotspots_on_map(self, folium_map, df_summary):
        """
        Render DBSCAN Hotspot Polygons / Circles highlighted in RED on Folium map.
        """
        if df_summary is None or df_summary.empty:
            return folium_map

        hotspot_group = folium.FeatureGroup(name="🚨 Hidden Hotspot DBSCAN Polygons", show=True)
        ...
        for _, row in df_summary.iterrows():
            ...
            color = '#ef4444' if is_hidden else '#f97316' # Bright red for hidden hotspots

            popup_html = f"""
            <div style="font-family:sans-serif; font-size:12px; color:#0f172a; width:220px;">
                <b style="color:{color}; font-size:14px;">{row['status_label']}</b><br>
                <hr style="margin:4px 0;">
                <b>Hotspot Cluster ID:</b> #{cid}<br>
                <b>Location:</b> {row['ward_name']}<br>
                <b>Multimodal Point Count:</b> {row['point_count']} signals<br>
                <b>Avg Severity Score:</b> {sev} / 100<br>
                <b>Data Modalities Fused:</b> {row['sources']}<br>
                <hr style="margin:4px 0;">
                <span style="color:#64748b; font-style:italic;">Recommendation: Dispatch mobile water mist cannon team & deploy micro-sensor.</span>
            </div>
            """
            ...
            folium.Circle(
                location=[lat, lon],
                radius=1800, # 1.8km radius polygon boundary
                color=color,
                weight=3,
                dash_array='6, 6',
                fill=True,
                ...
                tooltip=f"🚨 Hotspot #{cid} — {row['ward_name']}"
            ...
            folium.Marker(
                ...
                popup=folium.Popup(popup_html, max_width=240),
                icon=folium.DivIcon(
                    html=f"<div style='font-size:24px; color:{color}; text-shadow:0 0 10px {color}; font-weight:bold;'>🔥</div>",
